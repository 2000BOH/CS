...
export function M02Page({ complaints, rooms, onUpdate }: M02PageProps) {
    ...
    complaints.forEach(complaint => {
      const matchingRoom = rooms.find(room => {
        ...
        return c차수 === r차수 && c호실 === r호실 && 
               (room.숙박형태 === '장박_개인' || room.숙박형태 === '장박_법인');
      });
      ...
      if (matchingRoom) {
        ...
        result.push({
          ...
          조치사항: complaint.조치사항,
          기준날짜,
          담당자확인: complaint.담당자확인_M02 || false,
          원본데이터: complaint,
        });
      }
    });
  ...
  return (
    <div className="flex gap-4 items-start">
                ...
                {filteredTasks.map((task) => {
                  const rows = [
                    ...
                    >
                      ...
                      <td className="px-3 py-0.5 text-center whitespace-nowrap">
                        {expandedId === task.id ? (
                          <ChevronUp className="w-4 h-4 text-purple-600 mx-auto" />
                        ) : (
      ...
      <StaffDailyNotePanel staffId="03" staffName="시우" />
    </div>
  );
}